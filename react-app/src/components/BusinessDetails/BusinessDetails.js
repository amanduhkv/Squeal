...
const BusinessDetails = () => {
    ...
    console.log(">>>>>> REVIEWS", bizReviews)
    let numReviews;
    let numImages;
    if (bizReviews) {
    ...
    if (biz.start_time && biz.end_time) {
        ...
        hours = days.map(day => {
            return (
                <div className='single-business-hours'>{day} <span>{convertTime(biz.start_time)} - {convertTime(biz.end_time)} <span className={open ? open : 'none'}>{today === days.indexOf(day) + 1 ? open + " Now" : ""}</span></span></div>
            )
        })
    }
    let location = {
    ...
    if (bizReviews) {
        ...
        singleReview = reviewsArr.map(obj => {
            console.log(">>>>REVIEWSARR INDEX", obj)
